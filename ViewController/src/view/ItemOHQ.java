package view;

public class ItemOHQ {
    public ItemOHQ() {
        super();
    }
    private String childSKU;
    private String childSKUDescr;
    private String Color;
    private String itemSize;
    private String fit;
    private String itemType;
    private String season;
    private String inseam;
    private String category1;
    private String category2;
    private String category3;
    private int store1;
    private int store2;
    private int store3;
    private int store4;
    private int store5;
    private int store6;

    public void setStore1(int store1) {
        this.store1 = store1;
    }

    public int getStore1() {
        return store1;
    }

    public void setStore2(int store2) {
        this.store2 = store2;
    }

    public int getStore2() {
        return store2;
    }

    public void setStore3(int store3) {
        this.store3 = store3;
    }

    public int getStore3() {
        return store3;
    }

    public void setStore4(int store4) {
        this.store4 = store4;
    }

    public int getStore4() {
        return store4;
    }

    public void setStore5(int store5) {
        this.store5 = store5;
    }

    public int getStore5() {
        return store5;
    }

    public void setStore6(int store6) {
        this.store6 = store6;
    }

    public int getStore6() {
        return store6;
    }

    public void setStore7(int store7) {
        this.store7 = store7;
    }

    public int getStore7() {
        return store7;
    }

    public void setStore8(int store8) {
        this.store8 = store8;
    }

    public int getStore8() {
        return store8;
    }

    public void setWh1(int wh1) {
        this.wh1 = wh1;
    }

    public int getWh1() {
        return wh1;
    }

    public void setWh2(int wh2) {
        this.wh2 = wh2;
    }

    public int getWh2() {
        return wh2;
    }
    private int store7;
    private int store8;
    private int wh1;
    private int wh2;


  
    public void setInseam(String inseam) {
        this.inseam = inseam;
    }

    public String getInseam() {
        return inseam;
    }

    public void setCategory1(String category1) {
        this.category1 = category1;
    }

    public String getCategory1() {
        return category1;
    }

    public void setCategory2(String category2) {
        this.category2 = category2;
    }

    public String getCategory2() {
        return category2;
    }

    public void setCategory3(String category3) {
        this.category3 = category3;
    }

    public String getCategory3() {
        return category3;
    }

    public void setCollar(String collar) {
        this.collar = collar;
    }

    public String getCollar() {
        return collar;
    }
    private String collar;

    public void setChildSKU(String childSKU) {
        this.childSKU = childSKU;
    }

    public void setColor(String Color) {
        this.Color = Color;
    }

    public String getColor() {
        return Color;
    }

    public String getChildSKU() {
        return childSKU;
    }

    public void setItemSize(String itemSize) {
        this.itemSize = itemSize;
    }

    public String getItemSize() {
        return itemSize;
    }

    public void setChildSKUDescr(String childSKUDescr) {
        this.childSKUDescr = childSKUDescr;
    }

    public String getChildSKUDescr() {
        return childSKUDescr;
    }


    public void setFit(String fit) {
        this.fit = fit;
    }

    public void setSeason(String season) {
        this.season = season;
    }

    public String getSeason() {
        return season;
    }

    public String getFit() {
        return fit;
    }




    public void setItemType(String itemType) {
        this.itemType = itemType;
    }

    public String getItemType() {
        return itemType;
    }

}
